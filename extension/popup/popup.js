...
// Session ID — groups fast+ensemble calls for same text
...
async function runAnalysis(text, mode) {
  ...
  try {
    const response = await fetch(`${API_BASE}/predict`, {
      ...
      body: JSON.stringify({ text, mode, session_id: _sessionId }),  // mode passed exactly — no fallback
    });
    ...
    try {
      await fetch(`${API_BASE}/feedback`, {
        ...
        body: JSON.stringify({
          ...
          feedback: correct === 1 ? "correct" : "incorrect",
        }),
      });
    } catch (_) {}
